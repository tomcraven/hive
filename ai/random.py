...
def perform_move( board ):
	...
	has_tiles_to_place = len( board.get_unplayed_tiles() ) > 0

	must_place_tile = \
		( has_tiles_to_place ) and \
		(( len( board.get_played_tiles() ) == 0 ) or \
...
def perform_movement( board ):
	random_tile_in_play = random.choice( board.get_played_tiles() )
...
def no_available_moves( board ):
	tiles_in_play = board.get_played_tiles()
